...
- [x] **Agent Prompts** - Agent prompts stored in text files in /src/barnabeenet/prompts/ (meta_agent, instant_agent, action_agent, interaction_agent, memory_agent). Edit directly in files.
...
- [x] **Self-Improvement Dashboard Enhancements Phase 2** - Complete UI/UX upgrade per enhancement plan: (1) **Phase 6 - opusplan Model Mode**: Default model changed to "opusplan" which uses Opus for diagnosis/planning and Sonnet for implementation. Model selection UI updated with 3 options. (2) **Phase 5 - Enhanced Notifications**: New `_notify_phase_change()` method with phase-specific messages and priority flags. Notifications include session info and safety scores. (3) **Phase 1 - Home Page Integration**: SI Sessions card on dashboard home showing active sessions with status icons. Clickable to navigate directly to self-improve page. 10-second auto-refresh. (4) **Phase 4 - Timeline & Action Banner**: Visual progress timeline showing 7 steps (Started→Diagnosing→Plan Approval→Implementing→Testing→Commit Approval→Completed). Animated action-required banner when user input needed. Needs-attention styling on timeline steps. (5) **Phase 3 - CLI Output Terminal**: Full CLI terminal view with controls (auto-scroll toggle, show stderr toggle, clear, copy). Styled like real terminal with proper scrolling. (6) **Phase 2 - Entry Point Integration**: "Fix with Self-Improve" button in correction modal starts session with trace context. "Fix This" button in chat quick actions starts session with conversation context. Both use opusplan model and pass source tracking. (7) **Phase 7 - CLI Output API**: New `/sessions/{session_id}/cli-output` endpoint returns full CLI history (messages, operations, thinking). 690 tests pass.
