...
INSTANT_PATTERNS: list[tuple[str, str]] = [
    ...
    # Simple factual questions (fast answers, no memory needed)
    ...
    (r"^do i need (an |)umbrella( today)?(\?)?$", "weather"),
    (r"^(should i|do i need to) bring (an |)umbrella(\?)?$", "weather"),
    (r"^(what('s| is) the |)weather (like )?(tomorrow|this weekend)(\?)?$", "weather"),
